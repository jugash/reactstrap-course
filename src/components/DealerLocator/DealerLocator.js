...
import {Row, Col, Form, FormGroup, Input, ListGroup, ListGroupItem,
 Badge, Table, Button, InputGroup, InputGroupAddon} from 'reactstrap'
 ...
 class DealerLocator extends Component {
     ...
     componentDidMount() {
         Axios.get('http://localhost:3001/dealerships')
            .then((res) => {
                let stateCounter = res.data.reduce((dealerStateCount, dealer) => {
     ...
     onListClick(eventData) {
         ...
         this.setState({
             searchTerm : stateClicked
         })
     }

     handleInputChange(eventData) {
        this.setState({
            searchTerm : eventData.target.value
        })
     }
     ...
     render() {
        if(this.state.dealerships) {
            ...
            let searchBar = <div>
                <h1>Over {this.state.dealerships.length} Authorised dealers nationwide</h1>
                <Row>
                    <Col sm={12} md={{size : 6, offset : 3}}>
                        <Form>
                            <FormGroup>
                                <InputGroup>
                                    <Input type="text" onChange={this.handleInputChange} value={this.state.searchTerm} name="user-address" 
                                        placeholder="We are probably closer than you think. What state are you in?"/>
                ...
                if( this.state.stateCounter) {
                    stateCounterMarkup = <Row>
                        <Col sm={12} md={{size: 10, offset : 1}}>
                            <ListGroup>
                                {
                                    Object.keys(this.state.stateCounter).sort().map((key, i) => {
